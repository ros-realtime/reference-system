...
#ifndef REFERENCE_SYSTEM__NODES__RCLC_TRIGGER__FUSION_HPP_
#define REFERENCE_SYSTEM__NODES__RCLC_TRIGGER__FUSION_HPP_
#include <chrono>
#include <string>
#include <utility>
...
private:
  static void input_callback_0(const void * msgin, void * context)
  {
    ...
    if (this_ptr != NULL) {
      // msgin pointer (sub_msg_0) could already be used again for next data by rclc-executor 
      // until callback_1 is called!
      this_ptr->subscriptions_[0].cache = static_cast<const message_t *>(msgin);
      if (this_ptr->subscriptions_[0].cache != NULL) {printf(" data new ");} else { printf(" data NULL ");}
      printf(" called ");
    }
    printf("\n");
  }
  ...
  static void input_callback_1(const void * msgin, void * context)
  {
    ...
    if (this_ptr != NULL) {
        ...
        (this_ptr->subscriptions_[1].cache == NULL))
      {
        ...
        if (this_ptr->subscriptions_[0].cache != NULL) { status_0 = 1;}
        if (this_ptr->subscriptions_[1].cache != NULL) { status_1 = 1;}
        printf(
        "Error: data not available: input_0 %d input_1 %d \n", status_0, status_1);
        return;
      }
...
#endif  // REFERENCE_SYSTEM__NODES__RCLC_TRIGGER__FUSION_HPP_
