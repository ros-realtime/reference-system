...
int main(int argc, char ** argv)
{
  ...
  std::set<std::string> fusion_nodes = {"PointCloudFusion",
    "RayGroundFilter",
    // "EuclideanClusterDetector" is a special case and
    // prioritized on the callback-group level.
  ...
  fusion_exe.add_callback_group(
    euclidean_node->get_callback_group_of_subscription("/RayGroundFilter"),
    euclidean_node->get_node_base_interface());

  other_exe.add_node(nodes.at("EuclideanClusterDetector"));
